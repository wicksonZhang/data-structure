...
* 我们通过如上面 `Person` 案例来说明，为什么当对象重写了 `hashCode()` 之后一定要重写 `equals()`?
...
* 如果不重写 `Person` 的 `hashCode()` 和 `equals()` 最后 `hashMap.size()` 打印的结果为 3，因为我们将对象作为 key 内存地址作为 key 所以得出了 3.
...
* 如果重写 `Person` 的 `hashCode()` 和 `equals()` 最后 `hashMap.size()` 打印的结果为 2，因为 `person1` Key 会把 `person` 的 Key 覆盖掉。
